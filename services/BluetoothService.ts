...
class BluetoothService {
  ...
  private handleDeviceResponse(base64Data: string): void {
    ...
    try {
      ...
      console.log(`[${this.now()}] [RX] <- HEX=${hexString} len=${bytes.length}`);
      ...
      if (hexString.startsWith('7E7F67') && hexString.endsWith('FBFD')) {
        ...
        if (mode) {
          console.log(`[${this.now()}] [PARSE] 模式确认 mode=${mode}`);
          // 触发模式更新回调
        ...
        console.log(`[${this.now()}] [PARSE] 转速 speed=${speed} (hex=${hexString.substring(6, 14)})`);
        // 触发转速更新回调
        ...
        if (data) {
          console.log(`[${this.now()}] [PARSE] 综合数据 mode=${data.mode} speed=${data.speed} pressure=${data.pressure}`);
  ...
  private handleNotification(base64Data: string): void {
    ...
    try {
      ...
      while (true) {
        ...
        console.log(`[${this.now()}] [FRAME] 完整帧=${frame}`);
